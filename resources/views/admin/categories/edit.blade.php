...
<div class="main-content group-data-[sidebar-size=lg]:xl:ml-[calc(theme('spacing.app-menu')_+_16px)] group-data-[sidebar-size=sm]:xl:ml-[calc(theme('spacing.app-menu-sm')_+_16px)] px-4 group-data-[theme-width=box]:xl:px-0 ac-transition">
    ...
    <!-- Start Edit Course Category -->
    <form id="categoryForm" name="categoryForm" action="{{ route('categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="grid grid-cols-12 gap-x-4">
                ...
                <div class="p-1.5">
                    ...
                    <div class="grid grid-cols-2 gap-x-4 gap-y-5 mt-7 pt-0.5">
                        <div class="col-span-full xl:col-auto leading-none">
                            ...
                            <input type="text" id="name" name="name" placeholder="Category Name" class="form-input" value="{{ $category->name }}">
                            <p class="text-red-500 text-xs mt-1 hidden" id="name-error"></p> <!-- Placeholder for name error -->
                        </div>
                        <div class="col-span-full xl:col-auto leading-none">
                            <label for="icon" class="form-label">Category Icon *</label>
                            <input value="{{ $category->icon ?? '' }}" type="text" id="icon" name="icon" placeholder="Category Icon Name" class="form-input">
            ...
            <div class="col-span-full lg:col-span-5 card p-4 lg:p-6">
                <div class="p-1.5">
                        ...
                        <label for="thumbnailsrc" class="file-container ac-bg text-xs leading-none font-semibold mb-3 cursor-pointer aspect-[4/1.5] flex flex-col items-center justify-center gap-2.5 border border-dashed border-gray-900 dark:border-dark-border-four rounded-10 dk-theme-card-square">
                            ...
                            <span class="flex-center flex-col peer-[.uploaded]/file:hidden">
                                <span class="size-10 lg:size-15 flex-center bg-primary-200 dark:bg-dark-icon rounded-50">
                                    <img src="{{ $category->image ? asset('uploads/categories/' . $category->image) : asset('dashboard/assets/images/icons/upload-file.svg') }}"
                                    alt="icon"
                                    class="dark:brightness-200 dark:contrast-100 w-1/2 lg:w-auto">
                                </span>
                    ...
                    <button type="submit" id="submitButton" class="btn b-solid btn-primary-solid btn-lg w-max">Update Category</button>
                </div>
            </div>
        </div>
    </form>
    <!-- End Edit Course Category -->
</div>
@endsection
...
<script type="text/javascript">
    $("#categoryForm").submit(function(e){
        ...
        $.ajax({
            ...
            error: function(xhr) {
                $("#categoryForm button[type='submit']").prop('disabled', false); // Re-enable the button on error
            }
        });
    });
</script>
